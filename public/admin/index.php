...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    try{
        if($action==='add_person'){
            ...
            $stmt=$pdo->prepare("INSERT INTO people(cpf,name,birth_date,active) VALUES(?,?,?,1)
...
$appointments=$pdo->query("SELECT p.name,p.cpf,p.birth_date,d.service_date,s.service_time,a.booked_at
FROM appointments a JOIN people p ON p.id=a.person_id JOIN scheduling_slots s ON s.id=a.slot_id JOIN scheduling_days d ON d.id=s.scheduling_day_id
WHERE a.status='active' ORDER BY d.service_date,s.service_time,a.booked_at")->fetchAll();
$people=$pdo->query("SELECT p.id,p.cpf,p.name,p.birth_date,p.active,a.id appointment_id,d.service_date,s.service_time,a.booked_at
FROM people p
LEFT JOIN appointments a ON a.person_id=p.id AND a.status='active'
LEFT JOIN scheduling_slots s ON s.id=a.slot_id
LEFT JOIN scheduling_days d ON d.id=s.scheduling_day_id
ORDER BY (a.id IS NULL) DESC,p.name,p.cpf")->fetchAll();
?>
...
<section class="panel"><h2>Cadastrar pessoa para agendamento</h2><p class="muted">O acesso público será feito com CPF e data de nascimento.</p><form class="form-grid" method="post"><input type="hidden" name="_token" value="<?=e(csrfToken())?>"><input type="hidden" name="action" value="add_person"><input type="text" name="cpf" inputmode="numeric" placeholder="CPF" required><input type="text" name="name" placeholder="Nome completo" required><input type="date" name="birth_date" required><button class="primary" type="submit">Cadastrar pessoa</button></form></section>

<section class="panel"><h2>Pessoas cadastradas</h2><p class="muted">Quem estiver <strong>LIVRE</strong> poderá entrar com CPF + nascimento e escolher um horário.</p><div class="table-wrap"><table class="admin-table"><thead><tr><th>Nome</th><th>CPF</th><th>Nascimento</th><th>Situação</th><th>Agendamento</th><th>Cadastro</th><th>Ação</th></tr></thead><tbody>
<?php foreach($people as $p):?><tr><td><?=e($p['name'])?></td><td><?=e($p['cpf'])?></td><td><?=e(formatDateBr($p['birth_date']))?></td><td><span class="badge <?=isset($p['appointment_id'])?'badge-booked':'badge-free'?>"><?=isset($p['appointment_id'])?'AGENDADO':'LIVRE'?></span></td><td><?=isset($p['appointment_id'])?e(formatDateBr($p['service_date']).' às '.substr($p['service_time'],0,5)):'Ainda não escolheu horário'?></td><td><?=((int)$p['active']===1)?'Ativo':'Inativo'?></td><td><form method="post"><input type="hidden" name="_token" value="<?=e(csrfToken())?>"><input type="hidden" name="action" value="toggle_person"><input type="hidden" name="person_id" value="<?=(int)$p['id']?>"><button class="small-btn"><?=((int)$p['active']===1)?'Desativar':'Ativar'?></button></form></td></tr><?php endforeach;?><?php if(!$people):?><tr><td colspan="7">Nenhuma pessoa cadastrada.</td></tr><?php endif;?></tbody></table></div></section>
...
<section class="panel"><h2>Agendamentos ativos</h2><div class="table-wrap"><table class="admin-table"><thead><tr><th>Data</th><th>Hora</th><th>Nome</th><th>CPF</th><th>Nascimento</th><th>Gravado em</th></tr></thead><tbody><?php foreach($appointments as $a):?><tr><td><?=e(formatDateBr($a['service_date']))?></td><td><?=e(substr($a['service_time'],0,5))?></td><td><?=e($a['name'])?></td><td><?=e($a['cpf'])?></td><td><?=e(formatDateBr($a['birth_date']))?></td><td><?=e((new DateTimeImmutable($a['booked_at']))->format('d/m/Y H:i'))?></td></tr><?php endforeach;?><?php if(!$appointments):?><tr><td colspan="6">Nenhum agendamento ativo.</td></tr><?php endif;?></tbody></table></div></section></div></body></html>
